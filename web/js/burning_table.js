...
/*-----------------------弹出历史记录查询对话框界面---------------------------*/
function open_burningtable() {
    var diag = new Dialog();
    diag.Width = 700;
    diag.Height = 610;
    diag.Title = "参数变化及报警记录";
    diag.URL = "Burning_Tetable.html";
...
var burning_alop_Table_t1;
function burning_alop_flush(){

    clearInterval(burning_alop_Table_t1);
    ...
    burning_alop_Table_t1 = setInterval(burning_alop_flush,30000);
}
...
//无
    ...
    xmlhttp.open("GET","process.do?method=HD&process=fired&start="+startdate+" "+starttime+"&end="+enddate+" "+endtime,true);
    // xmlhttp.open("GET","http://192.168.10.212:8080/Process/process.do?method=HD&process=fired&start="+startdate+" "+starttime+"&end="+enddate+" "+endtime,true);
    xmlhttp.overrideMimeType('text/json; charset=utf-8');
    xmlhttp.send();
}
    ...
    for(var i=0;i<dataArr.length;i++) {
        ...
        var tdAlarmtype = document.createElement('td');
            ...
            if (SplitArr[2].match(prolineReg) && praName == "") {
                ...
                var operateTbody = document.getElementById('burningHDtbody');
                operateTbody.appendChild(tr);
            }

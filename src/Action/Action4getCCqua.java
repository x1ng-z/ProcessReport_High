 ...
 * @date 2020/9/3 8:33
 */
public class Action4getCCqua implements Callable<JSONArray> {
    ...
    public Action4getCCqua(HttpServletRequest httpServletRequest, ServletContext servletContext) {
       ...
       long starttime=System.currentTimeMillis();
        ...
        for(Firm firm:processMgr.getFirmmmaping().values()){
            pool.execute(new Runnable() {
                @Override
                public void run() {
                    JSONObject firmresult=OracleMESdb_Access_Data.getCCqua(firm,startdate,enddate);

 ...
 * @date 2020/9/3 8:33
 */
public class Action4getCMqua implements Callable<JSONArray> {
    ...
    public Action4getCMqua(HttpServletRequest httpServletRequest, ServletContext servletContext) {
       ...
       long starttime=System.currentTimeMillis();
        ...
        for(Firm firm:processMgr.getFirmmmaping().values()){
            pool.execute(new Runnable() {
                @Override
                public void run() {
                    JSONObject firmresult=OracleMESdb_Access_Data.getCMqua(firm,startdate,enddate);
